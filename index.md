...
|속성|영문명칭|설명|비고|  
|:----:|:----:|:----:|:----:|  
|판매 금액|Sell|상점에 판매하는 가격|ㄴㅇ|  
|구매 금액|Buy|상점에서 구매하는 가격|ㄴㅇ|  
|작물|Crops|씨앗을 심고 난 후 자란 오브젝트|ㄴㅇ|  
...
### 8. 개발 요구사항  
-  시작화면, 게임화면, 캐릭터 생성 화면, 불러오기 화면 총 4개의 화면이 있다.  
-  시작화면에는 새로시작하기, 불러오기, 종료하기 버튼 3개가 있다.  
-  새로 시작하기 버튼을 클릭 시 캐릭터 생성 화면으로 이동한다.  
- 불러오기 버튼을 클릭 시 불러오기 화면으로 이동한다.  
- 종료하기 버튼을 클릭 시 게임이 종료된다.  
- 캐릭터 생성 화면에서는 화면 중앙을 기준으로 양 옆에 캐릭터 두명을 보여주며 화면 중앙 하단에 닉네임을 입력하는 입력칸이 표시되며, 닉네임 저장 버튼과 생성하기 버튼이 있으며 누르면 게임화면으로 변경된다.  
- 불러오기 화면에서는 화면 중앙에 저장되어있는 캐릭터 정보들을 표시해주며 시작하기 버튼이 있으며 누르면 게임화면으로 변경된다.  

#### 게임화면
- 게임화면에는 왼쪽 상단에는 환경설정 아이콘, 오른쪽 상단에는 게임 내 날짜, 시간, 소지금액, 중앙 하단에는 미리보기 인벤토리 5칸, 오른쪽 하단에는 가방 아이콘, 에너지 게이지, 허기 게이지 UI가 표시된다.  
- 게임화면의 중간에는 플레이어가 비춰지고, 기본적으로 화면은 플레이어 후방 위쪽에 존재한다.  
- 게임화면에서 마우스로 클릭 후 상, 하, 좌, 우로 이동시 카메라가 그 방향에 따라 회전한다.  
- 맵은 플레이어 집, 숲, 마을 총 3개가 있다.  
- 플레이어 집에는 상호작용 가능한 오브젝트로 침대, 솥이 있다.  
- 침대에서 F키를 눌러 상호 작용시 수면 애니메이션을 실행한다.  
- 솥에서 F키를 눌러 상호 작용시 제작 탭이 표시된다.  
- 제작 탭에는 화면 오른쪽에는 레시피, 왼쪽에는 제작중인 아이템 정보, 중앙 상단에는 현재 선택한 레시피 정보, 왼쪽 하단에는 제작에 필요한 아이템UI를 표시하며 그 옆에는 제작하기 버튼이있다.  
- 제작하기 버튼을 클릭 시 화면 중앙에 제작할 아이템 개수를 입력 받는 창이 나오며 칸 아래에 제작하기 버튼이 표시된다.  
- 수량 입력 후 제작하기 버튼을 클릭 시 창이 사라지며 제작 탭 왼쪽 제작중인 아이템 정보에 제작한 아이템 정보 표시한다.  
- 제작이 완료되면 게임 화면 왼쪽 상단에 완료된 아이템을 표시해준다.  
- 마을에는 npc 3명이 있으며 음식점, 씨앗 상점, 목축장이 있다.  
- 각 npc앞에서 F키를 눌러 상호작용 시 상점 UI가 표시된다.  
- 상점 UI에는 화면 중앙을 기준으로 왼쪽에는 상점에서 판매하는 아이템과 구매 버튼, 오른쪽에는 플레이어의 인벤토리와 판매버튼이 표시된다.  

#### 캐릭터  
- 키보드 상, 하, 좌, 우로 이동이 가능하다.  
- 기본적으로 캐릭터 오른쪽에 구체가 떠다니며 캐릭터가 이동, 회전하면 같이 이동한다.  
- 기본적으로 캐릭터가 이동하는 방향은 카메라가 바라보는 방향이며 캐릭터가 이동 중 카메라가 회전하면 캐릭터가 바라보는 방향도 카메라가 바라보는 방향으로 바뀐다.  
- 구체를 클릭 시 구체를 중심으로 밭갈기, 씨앗 심기, 물 뿌리기, 수확하기, 삭제하기 총 5개 버튼 UI가 표시된다.  
- 밭 갈기 클릭 시 구체의 색깔이 갈색으로 변하며 마우스를 따라다니는 밭 오브젝트가 표시된다.  
- 이 밭 오브젝트는 밭이 생성 가능할 때 노란색, 불가능할때는 붉은색으로 변한다.  
- 생성 가능한 곳에 클릭 시 그 곳에 밭이 생성된다.  
- 씨앗 심기 버튼을 클릭 시 구체의 색깔이 초록색으로 변한다.  
- 아무것도 없는 밭을 클릭 시 클릭한 밭에 씨앗을 심는다.  
- 물 뿌리기 버튼을 클릭 시 구체의 색깔이 파란색으로변하며 씨앗이 심겨져 있는 밭을 클릭 시 물을 뿌리며 밭 색깔을 진한 갈색으로 바꾼다.  
- 수확하기 버튼을 클릭 시 구체의 색깔이 주황색으로 변하며 다 자란 작물을 클릭 시 작물의 종류에 따라 수확한다.  
- 수확 후 다시 자라나는 작물이라면 작물을 삭제하지 않고 작물 크기가 줄어들며 작물 중심으로 열매가 작물 주변으로 떨어진다.  
- 수확 후 사라지는 작물이라면 클릭 시 작물을 삭제하고 주변에 열매가 떨어진다.  
- 삭제하기 버튼을 클릭 시 구체가 검은색으로 바뀌며 밭, 작물을 클릭 시 삭제한다.  
- 피로도, 허기 게이지가 0일때 캐릭터는 쓰러짐 상태로 바뀐다.  
- 피로도 게이지가 20아래로 떨어지면 피곤함 상태로 바뀐다.  
- 허기짐 게이지가 20아래로 떨어지면 배고픔 상태로 바뀐다.  

#### 인벤토리  
- 인벤토리 아이콘 클릭 또는 키보드 I키를 누를 시 화면 중앙에 인벤토리 UI를 표시한다.  
- 인벤토리 UI에는 8 : 2비율로 왼쪽엔 인벤토리칸이 있고 오른쪽에는 쓰레기통 UI가 표시된다.  
- 인벤토리에 있는 아이템을 클릭 후 비어있는 인벤토리 칸을 클릭하면 클릭한 칸으로 아이템을 이동시킨다.  
- 인벤토리에 있는 아이템을 클릭 후 비어있지 않은 인벤토리 칸을 클릭 시 서로 위치를 바꾼다.  
