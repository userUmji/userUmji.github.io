...
2-13 각 npc앞에서 F키를 눌러 상호작용 시 상점 UI가 표시된다.  
2-14 상점 UI에는 화면 중앙을 기준으로 왼쪽에는 상점에서 판매하는 아이템과 구매 버튼, 오른쪽에는 플레이어의 인벤토리와 판매버튼이 표시된다.  


    
3-1 키보드 상, 하, 좌, 우로 이동이 가능하다.  
3-2 기본적으로 캐릭터 오른쪽에 구체가 떠다니며 캐릭터가 이동, 회전하면 같이 이동한다.  
3-3 기본적으로 캐릭터가 이동하는 방향은 카메라가 바라보는 방향이며 캐릭터가 이동 중 카메라가 회전하면 캐릭터가 바라보는 방향도 카메라가 바라보는 방향으로 바뀐다.  
3-4 구체를 클릭 시 구체를 중심으로 밭갈기, 씨앗 심기, 물 뿌리기, 수확하기, 삭제하기 총 5개 버튼 UI가 표시된다.  
3-5 밭 갈기 클릭 시 구체의 색깔이 갈색으로 변하며 마우스를 따라다니는 밭 오브젝트가 표시된다.  
3-6 이 밭 오브젝트는 밭이 생성 가능할 때 노란색, 불가능할때는 붉은색으로 변한다.  
3-7 생성 가능한 곳에 클릭 시 그 곳에 밭이 생성된다.  
3-8 씨앗 심기 버튼을 클릭 시 구체의 색깔이 초록색으로 변한다.  
3-9 아무것도 없는 밭을 클릭 시 클릭한 밭에 씨앗을 심는다.  
3-10 물 뿌리기 버튼을 클릭 시 구체의 색깔이 파란색으로변하며 씨앗이 심겨져 있는 밭을 클릭 시 물을 뿌리며 밭 색깔을 진한 갈색으로 바꾼다.  
3-11 수확하기 버튼을 클릭 시 구체의 색깔이 주황색으로 변하며 다 자란 작물을 클릭 시 작물의 종류에 따라 수확한다.  
3-12 수확 후 다시 자라나는 작물이라면 작물을 삭제하지 않고 작물 크기가 줄어들며 작물 중심으로 열매가 작물 주변으로 떨어진다.  
3-13 수확 후 사라지는 작물이라면 클릭 시 작물을 삭제하고 주변에 열매가 떨어진다.  
3-14 삭제하기 버튼을 클릭 시 구체가 검은색으로 바뀌며 밭, 작물을 클릭 시 삭제한다.  
3-15 피로도, 허기 게이지가 0일때 캐릭터는 쓰러짐 상태로 바뀐다.  
3-16 피로도 게이지가 20아래로 떨어지면 피곤함 상태로 바뀐다.  
3-17 허기짐 게이지가 20아래로 떨어지면 배고픔 상태로 바뀐다.  


    
4-1 인벤토리 아이콘 클릭 또는 키보드 I키를 누를 시 화면 중앙에 인벤토리 UI를 표시한다.  
4-2 인벤토리 UI에는 8 : 2비율로 왼쪽엔 인벤토리칸이 있고 오른쪽에는 쓰레기통 UI가 표시된다.  
4-3 인벤토리에 있는 아이템을 클릭 후 비어있는 인벤토리 칸을 클릭하면 클릭한 칸으로 아이템을 이동시킨다.  
4-4 인벤토리에 있는 아이템을 클릭 후 비어있지 않은 인벤토리 칸을 클릭 시 서로 위치를 바꾼다.  
4-5 인벤토리에 있는 아이템을 클릭 후 쓰레기통 UI를 클릭하면 아이템을 삭제한다.
